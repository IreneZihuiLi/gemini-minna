...
// Canonical audio lives in public/audio (generated by scripts/generate_gemini_audio.py)
// and is shared by the web app; each lesson subpackage gets a copy of the clips it needs.
...
// Mini program subpackages must stay small (2 MB each), so the shared clips are
// re-encoded at 24 kbps into a cache and copied from there.
const miniAudioBitrate = process.env.MINI_AUDIO_BITRATE || "24k";
const miniAudioCache = path.join(root, "node_modules", ".cache", `miniprogram-audio-${miniAudioBitrate}`);
fs.mkdirSync(miniAudioCache, { recursive: true });

function miniClip(source) {
  const cached = path.join(miniAudioCache, path.basename(source));
  const srcStat = fs.statSync(source);
  if (fs.existsSync(cached) && fs.statSync(cached).mtimeMs >= srcStat.mtimeMs) return cached;
  const tmp = `${cached}.tmp.m4a`;
  execFileSync("ffmpeg", ["-y", "-loglevel", "error", "-i", source, "-c:a", "aac_at", "-b:a", miniAudioBitrate,
    "-ar", "24000", "-ac", "1", tmp]);
...
for (let lessonId = 1; lessonId <= 50; lessonId++) {
  ...
  for (const text of lessonTexts(lessonId, lessons, grammar, content)) {
    ...
    copyFile(miniClip(source), path.join(packageDir, "assets", "audio", filename));
...
fs.rmSync(path.join(miniRoot, "assets", "audio"), { recursive: true, force: true });

console.log(`Built ${subPackages.length} lesson subpackages from ${sourceAudioDir}.`);
